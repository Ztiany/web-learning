...
但是解析 JS 代码时对 var 和 function 关键字声明的对象的处理是不一样的：前者在解析时只是将声明提前了，但是赋值却依旧在原来的位置。
...
在处理 function 定义的 sayHi 函数时，sayHi 的声明被提前了，同时也对它进行了定义，所以在第二行调用 sayHi 时正常输出hi而不是报 sayHi is not defined
 ...
 */
console.log(sayHello);         // ƒ sayHi() { console.log("hi"); }
